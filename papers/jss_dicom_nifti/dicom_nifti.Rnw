...
being implicit or explicit is made.  Explicit VRs of type \code{\{OB,
  OF, OW, SQ, UT, UN\}} skip bytes seven and eight (counting from one
instead of zero), convert the next four bytes into an integer length
and read \code{length} number of objects from the DICOM file.

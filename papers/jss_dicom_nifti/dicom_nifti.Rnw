...
states how their hardware implements DICOM.  The DICOM standard
...
<<DICOM Abdo 01>>=
abdo <- dicomInfo(system.file(file.path("dcm", "Abdo.dcm"), 
                              package="oro.dicom"))
names(abdo)
abdo$hdr[1:5,]
...
<<DICOM Siemens 01>>=
dcm <- dicomInfo(system.file(file.path("dcm", "MR-sonata-3D-as-Tile.dcm"), 
                             package="oro.dicom"))
dim(dcm$img)
...
<<mniLR_nifti>>=
(mniLR <- readNIfTI(system.file(file.path("nifti", "mniLR.nii.gz"),
                                package="oro.nifti")))
aux.file(mniLR)
descrip(mniLR)
@ 
...
<<mniRL-read>>=
(mniRL <- readNIfTI(system.file(file.path("nifti", "mniRL.nii.gz"),
                                package="oro.nifti")))
@ 
...
<<ffd>>=
(ffd <- readNIfTI(system.file(file.path("nifti", "ffd.nii.gz"), 
                              package="oro.nifti")))
@ 
...
As seen in Figure~\ref{ffd-orthographic} the mid-axial, mid-saggital
...
indices, and the crosshairs will provide a spaital reference in each
...
<<zstat1>>=
(zstat1 <- readNIfTI(system.file(file.path("nifti", "zstat1.nii.gz"), 
                                 package="oro.nifti")))
@ 
...
for DICOM conversion.  In addition there are several \proglang{R}
...
construction of the NIfTI data volume.

\subsection{Example}

Using the 40~images from the \code{hk40} object (previously defined in
Section~\ref{sec:dicom-header}) it is strightforward to perform

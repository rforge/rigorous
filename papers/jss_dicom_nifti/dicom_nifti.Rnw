...
dim(dcmImage)
(dcmNifti <- dicom2nifti(dcmImage, dcm$hdr, aux.file=fname))
image(dcmNifti)
@ 

The first image is taken from the raw DICOM file, in mosaic format,
and displayed with the default margins \code{par(mar=(5,4,4,2)+0.1)}.
The second image is displayed after re-organizing the original DICOM
file into a 3D array and then converting it to NIfTI format.
...
offers a few improvements: merging of the header and image information
into one file (.nii), re-organization of the 348-byte fixed header
into more relevant categories and the possibility of extending the
header information.
...
been modified to accept \code{nifti} and \code{anlz} objects and
...
%\begin{figure}[!htbp]
%  \centering
%  \includegraphics*[width=0.5\textwidth]{mniLR.png}

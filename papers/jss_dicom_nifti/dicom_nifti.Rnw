...
header information are queried
...
<<DICOM Abdo 03>>=
abdo$hdr$value[abdo$hdr$name == "Manufacturer"]
...
<<NIfTI-datatypes>>=
cbind(2^(1:5), convert.datatype(2^(1:5)))
@ 
...
be achieved with any visualization for arrays in R. For example, the
\pkg{EBImage} package provides functions \code{display} and
\code{animate} for visualization \citep{EBImage}.  Please note that
\pkg{EBImage} functions expect grey values in the range $[0,1]$, hence
a proper display of NIfTI images can be derived by
...
at this point in time.  Popular viewers, especially for brain imaging,
are
\begin{itemize}
...
and creates a 3D image.  Four-dimensinoal image volumes (3D in space
plus time) are also converted automatically by specifying
\code{DIM=4}.  Slice positions are taken from the
...
field is unique for each single slice of the volume.
...
patient~IDs.  In cases where a DICOM file includes images from more
than one series, the corresponding slices have to be chosen before
conversion, using \code{dicomTable}, \code{extractHeader}, and
\code{matchHeader}.
...
The authors would like to thank the anonymous institutions who have
made their DICOM data available in the public domain.  VS is supported

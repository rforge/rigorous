...
may be used to manipulate the data.  Several software packages allow
DICOM-to-NIfTI (or ANALYZE) conversion; e.g.,
\begin{itemize}
\item \pkg{oro.dicom} (\url{rigorous.r-forge.r-project.org})
\item \pkg{tractor.base} (\url{code.google.com/p/tractor})

...
<<DICOM Siemens 02,eval=FALSE,echo=FALSE>>=
...
and image are contained in a single file (\code{magic = "n+1\\0"}) or
two separate files (\code{magic = "ni1\\0"}), the latter being
...
Image information begins, in a single NIfTI file
(\verb+magic = "n+1\\0"+), after the first 352 bytes.  If header

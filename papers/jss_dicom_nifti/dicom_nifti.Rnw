...
<<DICOM Siemens 02,results=hide,eval=FALSE>>=
...
\begin{figure}[!htbp]
  ...
  \caption{\textbf{(a)} Single MOSAIC image.  \textbf{(b)} Lightbox
    display of three-dimensional array of images.  \label{fig:mosaic}}
\end{figure}
...
ANALYZE data format (\code{system.file("doc/ANALYZE75.pdf",
  package="oro.dicom")}).  The last four bytes in the NIfTI header
...
access the original file name of the file (if it exists) and the
...
\begin{figure}[!htbp]
  ...
  \caption{\textbf{(a)} Axial slices of MNI volume \code{mniLR\_nifti}
    ...
    volume \code{avg152T1\_RL\_nifti} stored and displayed in \emph{neurological} convention.}
\end{figure}

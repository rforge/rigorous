...
\Keywords{Medical Imaging, Data import/export, visualization, \proglang{R}}
\Plainkeywords{Medical Imaging, Data import/export, visualization, R} %% without formatting
...
Medicine; \url{http://medical.nema.org}) standard was developed from earlier
standards and released in 1993. It is the data format for clinical
imaging equipment and a variety of other devices whose complete
specification is beyond the scope of this paper.  All major
...
The list of header files can be converted into a data frame including all DICOM header files
using \code{dicomTable}, and saved for further analysis.
<<DICOM HK40 02>>=
hk40.info <- dicomTable(hk40$hdr)
writeHeader(hk40.info,file="hk40_header.txt")
sliceloc.col <- which(hk40$hdr[[1]]$name=="SliceLocation") 
print(as.numeric(hk40.info[,sliceloc.col])[1:5])
@ 
Single fields can also be extracted from the list. From this, 
images can be prepared for further procedures, e.g., by choosing 
relevant acquisition series or by extracting timing information.
<<DICOM HK40 03>>=
print(extractHeader(hk40$hdr,"SliceLocation")[1:5])
modality=extractHeader(hk40$hdr,"Modality",numeric=FALSE)
print(matchHeader(modality,"mr")[1:5])
tm<-extractHeader(hk40$hdr,"SeriesTime",numeric=FALSE)
print(str2time(tm[1:5]))
@ 
...
matching DICOM header fields with valid strings

<<DICOM Abdo 03>>=
abdo$hdr$value[abdo$hdr$name == "Modality"]
abdo$hdr$value[abdo$hdr$name == "RepetitionTime"]
abdo$hdr$value[abdo$hdr$name == "EchoTime"]
@ 
...
embedded within the single DICOM file using the \texttt{AcquisitionMatrix}
...
ANALYZE data format. The last four bytes in the NIfTI header correspond to the ``magic''
...
<<NIfTI-constructor>>=
(random.image <- nifti(array(runif(100*100),c(100,100,1)),datatype=convert.datatype()$FLOAT32))
print(random.image@dim)
print(dim(random.image))
@ 
Data types used for NIfTI formats can be achieved from the \code{convert.datatype} function.
<<NIfTI-datatypes>>=
convert.datatype(2^(1:5))
@ 

\code{writeNIfTI} outputs valid NIfTI class files, which can be opened in 
other medical imaging software. Files can either be stored as standard 
\texttt{.nii} files or compressed with gnuzip.
<<NIfTI-write,eval=TRUE,echo=TRUE>>=
...
Basic visualization of \code{nifti} and \code{anlz} class images can be achieved
with any visualization for arrays in R. For example, the \pkg{EBImage} package 
provides functions \code{display} and \code{animate} for visualization \citep{EBImage}.
Please note that \pkg{EBImage} functions expect grey values in the 
range $[0,1]$, hence a proper display of NIfTI images can be derived by
<<animate 01, eval=FALSE>>=
mniLR.range <- range(mniLR)
display((mniLR-mniLR.range[1])/diff(mniLR.range))
@

Interactive visualization of multidimensional arrays, stored in NIfTI
or ANALYZE format, is however best performed outside of \proglang{R} at this
...
Analysis of medical images typically leads to a voxel wise map of statistics,
e.g., z-statistics for a voxel-wise hypothesis test on activation in fMRI 
experiments \citep{Friston1994}. As convention, statistical maps are often 
displayed as overlay on a raw signal image.
This is an example of reading in and displaying a statistical map so
...
statistical map to an underlying structural image.
...
construction of the NIfTI data volume. \code{dicom2nifti}
automatically converts a list of DICOM files into an \code{nifti}
object, and \code{dicom2analyze} converts into an \code{anlz} object.
...
@ 
<<DICOM2NIFTI HK40 02,echo=TRUE,results=hide>>=
...
At default \code{dicom2nifti} takes all image data from the DICOM
list and creates a 3D image. 4D images (3D plus time) can be also
be converted automatically with option \code{DIM=4}. Slice positions
will be guessed from the \texttt{ImagePositionPatient} DICOM field. 
For example, using 
\code{DIM=4} on the code{hk40} DICOM data,
<<DICOM2NIFTI HK40 04,eval=FALSE>>=
(hk40n <- dicom2nifti(hk40,DIM=4))
@ 
will also produce a 3D image, as the \texttt{ImagePositionPatient} 
is unique for each single slice image.

\code{dicom2nifti} and \code{dicom2analyze} will fail, if the
dimensions of the single images in the DICOM list to not match. 
They do however not check for different series numbers or patient 
IDs. In cases where a DICOM file includes images from more than one
series, the corresponding slices have to be chosen before conversion,
using \code{dicomTable}, \code{extractHeader}, and \code{matchHeader}.

Data conversion from DICOM to NIfTI (or ANALYZE)
may also be performed using one of several standalone
...
The authors would like to thank... \textbf{WHO PROVIDED EXAMPLE DATASETS?}

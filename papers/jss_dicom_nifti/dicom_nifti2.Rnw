...
XML-based ``audit trail'' that tracks the history of operations
applied to a dataset.  The conversion from DICOM to ANALYZE/NIfTI is
straightforward using the capabilities of both packages.  The
\proglang{S}4 classes have been developed to provide a user-friendly
interface to the ANALYZE/NIfTI data formats; allowing easy data input,
...
produces an \proglang{S}4 \code{"nifti"} object (or \code{"niftiAuditTrail"} if
...
"n+1\textbackslash{}0"}), this is by default after the first 352 bytes.  Header
extensions extend the size of the header and come before the image information
leading to a consequent increase of \code{voxoffset} for single NIfTI files.
Split NIfTI files do not have this problem and \code{voxoffset} is set at 0.

The \code{image} function has been overloaded so that it behaves differently
when dealing with medical image objects (\code{nifti} and \code{anlz}).  The
command
...
medical image \proglang{S}4 objects is an attempt to balance minimal user input
...
the \proglang{S}4 system \citep{chambers:2008}.  Essentially, NIfTI and ANALYZE
...
<<NIfTI-constructor>>=
(random.image <- nifti(array(runif(100*100),c(100,100,1)), datatype=16))
random.image@"dim"
dim(random.image)
@ 
...
extension is converted into plain text and saved using \code{ecode =
  6} to denote plain ASCII text only.  The user may control the
tracking of data manipulation via the audit trail using a global
...
\subsection{fMRI example}% from the NIfTI-1 DFWG}
...
\texttt{filtered\_func\_data.nii.gz} obtained \textcolor{BrickRed}{
from the FSL evaluation and example data suite
(\url{http://www.fmrib.ox.ac.uk/fsl/fsl/feeds.html}).} Successful execution of
the commands
...
resolution ($3\;\text{s}$).  Figure~\ref{fig:ffd-orthographic} depicts
the orthographic display of the \texttt{filtered\_func\_data} using
the axial plane containing the left-and-right thalamus to
approximately center the crosshair.
...
\begin{figure}[tbp]
  \begin{center}
    \includegraphics*[width=\textwidth]{ffd_design.png}
    \end{center}
  \caption{Visual (30 seconds on/off) and auditory (45 seconds on/off) stimulus 
    convolved with a Gamma haemodynamic response function, used in the GLM fMRI
    analysis.}
...
\textcolor{BrickRed}{R provides virtually any statistical method for the 
analysis of imaging data. 
For example, fMRI data are typically analysed by fitting a general 
linear model (GLM) with using the stimuli to construct a design matrix.
This leads to a resulting statistical image, e.g., $Z$-statistics 
for a voxel-wise hypothesis test on activation in fMRI experiments
\citep{Friston1994}.}

\textcolor{BrickRed}{The \texttt{filtered\_func\_data.nii.gz} image was acquired in an 
experiment with $TR = 3s$ and both visual and 
auditory stimulus. The visual stimulus was a on/off with an amplitude of
60 seconds and the auditory stimulus was on/off with an amplitude of 90 
seconds. We use a Gamma haemodynamic respone function (HRF) with
mean 6 and standard deviation 3, similar to the default values in FSL.
Figure~\ref{fig:ffd-orthographic} depicts the convolved visual and auditory stimuli.}

<<ffd-glm-design,echo=TRUE,results=hide>>=
# visual (30s on/off) and auditory (45s on/off) stimulus
visual <- rep(c(rep(-.5,30),rep(.5,30)),12)
auditory <- rep(c(rep(-.5,45),rep(.5,45)),12)
# haemodynamic response function with mean=6 and sd=3
hrf <- c(dgamma(1:15,4,scale=1.5))
# convolve stimuli with hrf
visual.hrf <- rep(NA,540)
for (k in 60:599)visual.hrf[k-59] <- sum(visual[k-(1:15)] * hrf) 
auditory.hrf <-rep(NA,540)
for (k in 90:629)auditory.hrf[k-89] <- sum(auditory[k-(1:15)] * hrf) 
# add phase
visual.hrf <- c(rep(0.5,30),visual.hrf)
auditory.hrf <- c(rep(0.5,45),auditory.hrf)
# TR = 3s
visual.hrf <- visual.hrf[seq(3,540,by=3)]
auditory.hrf <- auditory.hrf[seq(3,540,by=3)]
# center
visual.hrf <- visual.hrf-mean(visual.hrf)
auditory.hrf <- auditory.hrf-mean(auditory.hrf)
@ 
<<ffd-glm-design.png,echo=FALSE,results=hide>>=
png(filename="ffd_glm_design", width=2*480, height=0.5*480)
par(mfrow=c(1,2))
plot(seq(1,540,by=3),visual.hrf,type="l",xlab="time (s)",ylab="visual stimulus")
plot(seq(1,540,by=3),auditory.hrf,type="l",xlab="time (s)",ylab="auditory stimulus") 
dev.off()
@ 

\textcolor{BrickRed}{The design matrix is than used in a voxel-wise GLM  using the the 
standard \texttt{lm()} \proglang{R} function for (general) linear models. From this,
for each voxel, e.g., t-statistics and p-values for a hypothesis test on no effect of each
stimulus along with a F-statistic for a hypothesis test on no effect of any stimuli can
be computed.}
...
{
# background threshold: 10% max intensity
  if (max(x)<thresh) return(rep(NA,5))
# glm
...
flim <- apply(ffd, 1:3, local.lm)
@ 

For anatomical context, statistical images are displayed as an overlay on
top of a reference image. Successful execution of
...
<<zstst1>>=
# t-statistics
t.visual <- nifti(flim[1,,,])
t.auditory <- nifti(flim[2,,,])
# p-values
p.visual <- nifti(flim[3,,,])
p.auditoy <- nifti(flim[4,,,])
# F-statistics
F <- nifti(flim[5,,,])

# Z-statistic: normalized t-statistic
Z.visual <- nifti(qnorm(pt(t.visual,179,log.p=TRUE),log.p=TRUE))
Z.auditory <- nifti(qnorm(pt(t.auditory,179,log.p=TRUE),log.p=TRUE))
@ 
<<zstat1-png,echo=FALSE,results=hide>>=
png("ffd_zstat1.png", width=2*480, height=2*480, bg="black")
@ 
<<zstst1-overlay>>=
overlay(ffd, ifelse(abs(Z.visual) > 5, Z.visual, NA), zlim.x=range(ffd), zlim.y=range(Z.visual,na.rm=TRUE))
...
@ 
<<zstst2-overlay>>=
overlay(ffd, ifelse(abs(Z.auditory) > 5, Z.auditory, NA), zlim.x=range(ffd), zlim.y=range(Z.auditory,na.rm=TRUE))
<<zstat2-dev.off,echo=FALSE,results=hide>>=
dev.off()
@ 

The four-dimensional array of parameter estimates are

...
\textcolor{BrickRed}{The design matrix is than used in a voxel-wise
  GLM using the the standard \code{lm} function in \proglang{R} for
...
<<zstst1>>=
## t-statistics
t.visual <- nifti(flim[1,,,], datatype=16)
t.auditory <- nifti(flim[2,,,], datatype=16)
## p-values
p.visual <- nifti(flim[3,,,], datatype=16)
p.auditoy <- nifti(flim[4,,,], datatype=16)
## F-statistics
F <- nifti(flim[5,,,], datatype=16)

# Z-statistic: normalized t-statistic
dof <- 179
Z.visual <- nifti(qnorm(pt(t.visual, dof, log.p=TRUE), log.p=TRUE),
                  datatype=16)
Z.auditory <- nifti(qnorm(pt(t.auditory, dof, log.p=TRUE), log.p=TRUE),
                    datatype=16)
@ 
...
The 4D array of parameter estimates for the visual task are overlayed
on the original data for ``anatomical'' reference in
Figure~\ref{fig:ffd+zstat1}b.  The function \code{overlay} extends the
capabilities of displaying two-dimensional images by allowing one to
add a statistical image to a reference image of the same dimension.
...
\code{nifti} object, and likewise \code{dicom2analyze} converts them
into an \code{anlz} object.

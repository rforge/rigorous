...
\pkg{oro.dicom} and \pkg{oro.nifti}.
\textcolor{BrickRed}{Both packages include data input and output.
S4 classes \texttt{nifti} and \texttt{anlz} are provided for further 
statistical analysis in \proglang{R} without loosing information 
from the images. Images in the metadata-rich DICOM format can 
be converted to NIfTI semi-automatically using as much 
information from the DICOM files as possible. Basic visualization 
functions similar to standards in the medical imaging community 
are provided for \texttt{nifti} and \texttt{anlz} objects.
Additionally, \pkg{oro.nifti} allows to track every operation on a
\texttt{nifti} object in an audit trail.}

\textcolor{BrickRed}{These packages should appeal not
only to other \proglang{R} package developers, but also to
scientists who want to manipulate and analyze medical imaging data using
\proglang{R} without writing and validating basic data input/output
functionality. An example of facilitating \proglang{R} for the analysis
of functional Magnetic Resonance Imaging (fMRI) is given in section \ref{fmri_example}.
Packages already available on CRAN that utilize
\pkg{oro.dicom} and \pkg{oro.nifti} include \pkg{cudaBayesreg}
\citep{cudaBayesreg}, \pkg{dcemriS4} \citep{dcemriS4},
\pkg{dpmixsim} \citep{dpmixsim}, and \pkg{RNiftyReg} \citep{RNiftyReg}.}
...
rich metadata at \url{http://cabig.nci.nih.gov/tools/NCIA}.  The

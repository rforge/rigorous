...
\begin{table}[tbp]
  ...
  \caption{List of functions available in \pkg{oro.dicom} and
    \pkg{oro.nifti}.  Functionality around the AFNI data format was
    recently added to the \pkg{oro.nifti} package.  Please visit
...
data format.  VS is supported by the LMU innovative project BioMed-S:

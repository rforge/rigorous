...
\proglang{XML}-based log \cite{XML}.  Each log entry contains
...
\subsection{Functional MRI Example}% from the NIfTI-1 DFWG}
...
the axial plane \textcolor{BrickRed}{containing the left-and-right
  thalamus to approximately center the crosshair vertically}.
...
\textcolor{BrickRed}{The 4D volume of imaging data in
  ...
  $TR=3\;\text{s}$, using both visual and auditory stimuli.  The
  visual stimulus was a on/off with a duration of 60 seconds and the
  auditory stimulus was on/off with a duration of 90 seconds.  We use
  a parametric haemodynamic respone function (HRF), with mean 6 and
  standard deviation 3, similar to the default values in FSL.
  Figure~\ref{fig:ffd-orthographic} depicts the visual and auditory
  stimuli convolved with the HRF.}

<<ffd-glm-design,echo=TRUE,results=hide>>=
# visual (30s on/off) and auditory (45s on/off) stimulus
visual <- rep(c(rep(-0.5, 30), rep(0.5, 30)), 12)
auditory <- rep(c(rep(-0.5, 45), rep(0.5, 45)), 12)
...
plot(index, visual.hrf, type="l", 
     xlab="Time (seconds)", ylab="Visual Stimulus")
plot(index, auditory.hrf, type="l",
     xlab="Time (seconds)", ylab="Auditory Stimulus") 
dev.off()
@ 

\textcolor{BrickRed}{The design matrix is than used in a voxel-wise
  GLM using the the standard \texttt{lm} function in \proglang{R} for
  parameter estimation in linear regression.  From this, for each
  voxel, $t$-statistics and associasted $p$-values for a hypothesis
  test on no effect of each stimulus along with an $F$-statistic for a
  hypothesis test on no effect of any stimuli can be computed.}

<<ffd-glm,echo=TRUE,results=hide>>=
## background threshold: 10% max intensity
T <- 0.1 * max(ffd) 
local.lm <- function(x, thresh) { # general linear model
  ...
  c(as.vector(summary(output)$coeff[2:3,3:4]), 
    summary(output)$fstatistic[1])
}
## apply local glm to each voxel
flim <- apply(ffd, 1:3, local.lm, thresh=T)
@ 

For anatomical context, statistical images are displayed as an overlay
on top of a reference image.  Successful execution of the command
allows one to display the statistical image of voxel-wise activations
overlayed on the original EPI (echo planar imaging) data acquired in
the fMRI experiment.
...
@ 
<<zstst1-overlay>>=
overlay(ffd, ifelse(abs(Z.visual) > 5, Z.visual, NA), 
...
@ 
<<zstst2-overlay>>=
overlay(ffd, ifelse(abs(Z.auditory) > 5, Z.auditory, NA), 
        zlim.x=range(ffd), zlim.y=range(Z.auditory, na.rm=TRUE))
<<zstat2-dev.off,echo=FALSE,results=hide>>=
dev.off()
@ 

The 4D array of parameter estimates are overlayed on the original data
for anatomical reference in Figure~\ref{fig:ffd+zstat1}b.  The
function \code{overlay} extends the capabilities of displaying
``images'' by allowing one to add a statistical image to a reference
image of the same dimension.
